...
for(i=0; i < 100; i++) buf[i] = '\0';
    ...
    printf("Enter roll number to get details: \n"); //ask details from client
    int x; 
    scanf("%d",&x); //store in variable

    struct student s; //make a structure
	  send(sockfd, &x, sizeof(int), 0); //send roll to server 
    recv(sockfd,&s,sizeof(struct student),0); //receive response
    printf("Roll:%d\nMark:%5.2f\nName:%s",s.roll,s.mark,s.name);
